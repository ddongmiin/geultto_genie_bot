...
한달동안 글또에서의 연장전을 달릴 여러분들을 위해 지난 5개월 동안의 여러분의 활동내역을 정리해보았습니다.👏
\n
> 🔎 *{row[14]}님의 활동 요약* 
    • {row[14]}님은 총 {row[3]}개의 글과 {row[4]}개의 쓰레드 {row[5]}개의 이모지를 남겨주셨어요. 
    • 지금까지 활동한 양을 보니 어떤 마음이 드시나요? 만약 아쉬운 마음이 든다면 연장된 1달동안 {row[14]}의 글과 생각을 글또에 남겨보시는건 어떠실까요?!🙏
\n
\n
> 📝 *{row[14]}님의 글 제출 내역*
    ...
    • {pass_text}
    • 이렇게 보니 5달이란 시간이 엄청 빨리 간것 같아요! 이렇게 끝내기 아쉽지 않나요? 아직 우리에겐 1달의 연장된 기간동안 글을 제출하셔서 추가 환급금을 받아가세용~! 가보자고~~💪🏻💪🏻
\n
\n
> 🐾 *{row[14]}님의 주요 활동 채널*
    1위: {channel_1}
...
☕️ *{row[14]}님의 커피챗 내역*
    ...
    • {coffee_text}
    • 글또가 끝나기전에 커피챗을 하며 글을 공유하고 이야기나눠보면 어떠실까요?☕️
\n
\n  
{final_ment} 
글또 9기동안 {row[9]}로서 활동하시며 좋은 글 남겨주셔서 감사드립니다.
남은 연장전도 열심히 참여해실거죠?! 그럼 저 데달부는 {row[14]}님의 남은 글또 활동도 응원하며 물러가보겠습니다. 저희는 글또 10기에서 더 발전된 모습으로 만나요!
    """
    print(row[1])
